...
async def ocr_space_file(
    filename, overlay=False, api_key=var.OCR_SPACE_API_KEY, language="eng"
):
...
async def download_video(v_url):
    ...
    if "audio" in dl_type:
        opts = {
            ...
            "outtmpl": os.path.join(
                var.TEMP_DOWNLOAD_DIRECTORY, str(s_time), "%(title)s.%(ext)s"
            ),
            "quiet": True,
            "logtostderr": False,
    ...
    if audio:
        ...
        thumb_image = [
            x
            for x in glob(os.path.join(var.TEMP_DOWNLOAD_DIRECTORY, str(s_time), "*"))
...
CMD_HELP.update(
    {
        ...
        \n  •  **Example :** `{cmd}google -l4 WayDev` atau `{cmd}google WayDev`\
    "
    }
)
