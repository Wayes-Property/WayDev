        ...
        if x.is_group:
            ...
            if chat not in GCAST_BLACKLIST and chat not in BLACKLIST_GCAST:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Berhasil Mengirim Pesan Ke** {done} **Grup.**\n**Gagal Mengirim Pesan Ke** {er} **Grup.**"
    )
        ...
        if x.is_user and not x.entity.bot:
            ...
            if chat not in DEVS:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Berhasil Mengirim Pesan Ke** {done} **Chat.**\n**Gagal Mengirim Pesan Ke** {er} **Chat.**"
    )
...
Ketik `{cmd}addblchat` di grup yang ingin anda tambahkan ke daftar blacklist gcast."""
        )
    else:
        await eod(event, "**🔮 Blacklist GCAST:** `Disabled`")


@ayiin_cmd(pattern="addblchat(?:\\s|$)([\\s\\S]*)")
async def add(event):
    me = await event.client.get_me()
    BLACKLIST_GCAST = AyiinChanger(cek_gcast(me.id))
...
@ayiin_cmd(pattern="delblchat(?:\\s|$)([\\s\\S]*)")
async def _(event):
    ...
    if gc in BLACKLIST_GCAST:
        del_gcast(me.id, gc)
        await xxx.edit(f"**Berhasil Menghapus Grup dari daftar blacklist gcast.**"
        )
    else:
        await eod(
            xxx,
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}addblchat`\
        \n  »  **Kegunaan : **Untuk Menambahkan grup tersebut ke blacklist gcast.\
        \n\n  »  **Perintah :** `{cmd}delblchat`\
        \n  »  **Kegunaan : **Untuk Menghapus grup tersebut dari blacklist gcast.\
        \n  •  **Note : **Ketik perintah** `{cmd}addblchat` **dan** `{cmd}delblchat` **di grup yang kamu Blacklist.\
    "
    }
)
