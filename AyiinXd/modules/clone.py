...
async def impostor(event):
    ...
    AyiinXd = ["@waymethodf", "@WayMethodd"]
    if inputArgs in AyiinXd:
        await eor(event, "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Way-Userbot Ngentod 😡")
        await event.client.send_message(
            "@WayChatss",
            "**Maaf Telah MengClone Way 🥺**"
        )
        return
    ...
    if inputArgs:
        ...
        if replyMessage.sender_id in DEVS:
            return await xx.edit(
                "**[ᴋᴏɴᴛᴏʟ]** - Tidak dapat menyamar sebagai Developer Way-Userbot Ngentod 😡"
            )
