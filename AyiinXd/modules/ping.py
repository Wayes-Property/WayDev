...
@ayiin_cmd(pattern="way$")
async def _(pong):
    await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    kopong = await edit_or_reply(pong, "**『Wᴀʏ-Usᴇʀʙᴏᴛ』**")
    await kopong.edit("**◆◈Oᴡɴᴇʀ Pᴀʟɪɴɢ Gᴀɴᴛᴇɴɢ😍◈◆**")
    await kopong.edit("**Uᴅᴀʜ Pᴜɴʏᴀ Cᴇᴡᴇ Jᴀɴɢᴀɴ Dɪ Gᴏᴅᴀɪɴ😡**")
    await kopong.edit("**☬WᴀʏDᴇᴠ Sɪᴀᴘ Mᴇᴍʙᴀsᴍɪ Kᴇʀᴏᴄᴏ🔥☬**")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
