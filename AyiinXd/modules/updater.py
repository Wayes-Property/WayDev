...
async def deploy(xx, repo, ups_rem, ac_br, txt):
    if HOSTED_ON == 'heroku':
        if var.HEROKU_API_KEY is not None:
            ...
            await eor(
                xx,
                "**✧ Wᴀʏ-Usᴇʀʙᴏᴛ ✧** `Berhasil Di Update Userbot bisa di gunakan kembali.`"
            )
...
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("**[•]** - Proses Update\n✧ Wᴀʏ-Usᴇʀʙᴏᴛ ✧ : `Menginstall Pembaruan...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await eod(xx, "**✧ Wᴀʏ-Usᴇʀʙᴏᴛ ✧ Sudah Versi Terbaru**")
        return repo.__del__()
...
CMD_HELP.update(
    {
        ...
        \n\n  »  **Perintah :** `{cmd}update`\
        \n  »  **Kegunaan : **Untuk Melihat Pembaruan Terbaru Way-Userbot.\
        \n\n  »  **Perintah :** `{cmd}update deploy`\
        \n  »  **Kegunaan : **Untuk MengUpdate Fitur Terbaru Dari Way-Userbot.\
    "
    }
)
